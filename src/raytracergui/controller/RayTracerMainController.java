...
public class RayTracerMainController {
	...
	private final ObservableList<raytracergui.enums.Camera> cameraNames = FXCollections
			.observableArrayList(raytracergui.enums.Camera.values());
	...
	private void initializeListeners() {
		...
		PlusMinusSlider[] nodeSliders = new PlusMinusSlider[] { transX, transY, transZ, scaleX, scaleY, scaleZ, translateX, translateY,
				translateZ };
		...
		for (final TextField l : cameraLabels) {
			...
			l.setOnKeyReleased((keyEvent) -> {
				try {
					...
					Notifications.create().position(Pos.TOP_RIGHT).title("Value not accepted").text(String.valueOf(e)).showError();
				}
			});
	...
	/**
	 * holt sich koordinaten und shuffelt diese
	 * 
	 * @param width
	 * @param height
	 */
